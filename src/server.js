...
const JARVIS_BUILDER_URL = process.env.JARVIS_BUILDER_URL?.trim()
  ?.replace(/\/+$/, "");
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      backupPath =
        `${p}.bak-${new Date().toISOString().replace(/[:.]/g, "-")}`;
...

      {
        ...
        options: [
          ...
          },
          {
            ...
            label: "Standard Plan (Global)",
          },
          {
            value: "modelstudio-standard-api-key-cn",
            label: "Standard Plan (CN)",
          },
        ],
      },
...
function validatePayload(
  ...
  if (
    RAILWAY_CONSOLE_AUTH_CHOICES.includes(
      payload.authChoice,
    )
  ) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (ok) {
        ...
        stream(
          `[config] gateway.auth.token exit=${tokenResult.code}\n`,
        );
        ...
        stream(
          `[config] gateway.trustedProxies exit=${proxiesResult.code}\n`,
        );
        ...
        if (
          ...
          stream(
            `[setup] Setting model to ${payload.model.trim()}...\n`,
          );
  ...
  async (_req, res) => {
    ...
    res.json({
      ...
      openclaw: {
        ...
        channelsAddHelpIncludesTelegram:
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          ...
          message:
            "Choose a channel and enter a pairing code.",

          error:
            "Missing channel or code",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          ...
          message:
            "Channel must be Telegram or Discord.",

          error:
            `Invalid channel: ${channelValue}`,
        });
    }
    ...
    if (
      /no pending pairing request/i.test(
        cleanOutput,
      )
    ) {
      return res
    ...
    return res
      ...
      .json({
        ...
        message:
          "Channel approval failed. Review the log output for details.",
  ...
  async (_req, res) => {
    try {
      ...
      res
        ...
        .send(
          "OK - stopped gateway and deleted config file. You can rerun setup now.",
        );
    } catch (err) {
      res
        .status(500)
  ...
  async (req, res) => {
    ...
    try {
        ...
        if (!exists) {
          return res
            ...
            .json({
              ...
              error:
                `Unknown pending device pairing request: ${targetRequestId}`,
            });
        }
      } else {
        const latest =
          newestPendingDevicePairing(
            pending,
          );
        ...
        if (!targetRequestId) {
          return res
            ...
            .json({
              ...
              error:
                "No pending device pairing requests.",
            });
        }
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            error:
              describeDeviceApprovalForbidden(
                result,
              ),
      ...
      return res.json({
        ...
        output:
          `Approved device pairing request ${targetRequestId}.`,
      });
    } catch (err) {
      const message =
        err?.message ||
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            error:
              "No data directories found to export.",
          });
      }
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          ...
          error:
            "Import only supported when state/workspace dirs are under /data.",
        });
    }
      ...
      try {
        ...
        return res
          ...
          .json({
            ...
            error:
              "Invalid X-Archive-Password header.",
          });
      }
    }
    ...
    const walkRejectSymlinks =
      (dir) => {
        ...
        if (
          ...
          throw reject(
            400,
            `Refusing symlink in archive: ${rel}`,
          );
        }
        ...
        if (
          ...
          throw reject(
            400,
            `Refusing non-regular entry in archive: ${rel}`,
          );
        }
      };
    ...
    try {
      ...
      if (
        ...
        throw reject(
          400,
          `Archive contains ${bad.length} entries outside ${STATE_DIR} / ${WORKSPACE_DIR}. First few: ${bad.slice(0, 3).join(", ")}`,
        );
      }
      ...
      if (
        ...
        log.error(
          "import",
          `unzip exit ${extractResult.code}: ${extractResult.output}`,
        );
      ...
      log.info(
        "import",
        `restored ${buf.length} bytes from archive`,
      );
...
const server =
  app.listen(
    ...
    () => {
      ...
      log.info(
        "wrapper",
        `setup wizard: http://localhost:${PORT}/setup`,
      );
      ...
      if (
        ...
        (
          ...
          (err) => {
            log.error(
              "wrapper",
              `failed to start gateway at boot: ${err.message}`,
            );
          },
        );
      }
...
async function gracefulShutdown(
  ...
  if (
    setupRateLimiter.cleanupInterval
  ) {
    clearInterval(
      setupRateLimiter.cleanupInterval,
    );
  ...
  if (gatewayProc) {
    try {
      ...
      log.warn(
        "wrapper",
        `error killing gateway: ${err.message}`,
      );
    }
  }
  ...
  try {
    ...
    log.info(
      "wrapper",
      `gateway stop during shutdown exit=${stopResult.code}`,
    );
  } catch (err) {
    log.warn(
      "wrapper",
      `gateway stop during shutdown failed: ${err.message}`,
    );
  }
